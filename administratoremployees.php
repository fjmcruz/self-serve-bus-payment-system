...
<body onload=clock();>
    ...
    <div id="containerone">
        <h2>EMPLOYEES:</h2>
        <form action="employeesearchaction.php" target="POPUP" onsubmit="POPUP=window.open('about:blank','POPUP','width=1000,height=600');">
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        if ($row["Username"] == $username) {
                            ...
                            echo "<td>" . "<a href='removeemployee.php?id=".$row["ID"]."' class='delete'>REMOVE</a>" . "</td></tr>";
                        }
                    }
                }
            ?>
        ...
        <div id="pagination">
            <?php
                if ($page > 1) {
                    echo "<a href='administratoremployees.php?page=".($page - 1)."'><</a>";
                }
                for ($i = max(1, $page - 5); $i <= min($page + 5, $totalpages); $i++) {
                    echo "<a href='administratoremployees.php?page=".$i."'";
