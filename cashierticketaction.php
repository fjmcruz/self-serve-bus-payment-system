<?php
    ...
    foreach ($Seat as $Value) {
        ...
        if (mysqli_num_rows($result) == 0) {
            echo "<script>window.location.href='cashierseaterror.php'</script>";
            exit();
        } 
    }
        if (in_array("A1", $Seat)) {
        ...
        $sql = "INSERT INTO tickettransactions (Year, Month, Date, Time, Source, Username, Ticket, Bus_Number, Route, Dispatch_Time, Dispatch_Date, Seats, Fare, Status)
                VALUES ('".$Year."', '".$Month."', '".$Date."', '".$Time."', '".$Source."', 'Cashier: ".$username."', '".$Ticket."', '".$Bus_Number."', '".$Route."', '".$Dispatch_Time."', '".$Dispatch_Date."', '".$Seat_String."', '".$Fare."', 'Cashier')";
        mysqli_query($connection,$sql);
        echo "<script>window.location.href='cashierhome.php'</script>";
        exit();
?>